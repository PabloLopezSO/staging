...
import com.example.demo.domain.ValidMimeTypesEnum;
...
public class FileService {
    ...
    private String taskNotPresentMessage = "task not present";
    private String emptySlotMessage = "empty slot";
    AzureQueueSyncServiceAClientConfig azureQueueConfig;

    @Autowired
    public FileService(QueueClient queueClient, TaskRepository todoAppRepository,
            @NonNull FileStorageProperties fileStorageProperties, BlobServiceClient blobServiceClient,  AzureQueueSyncServiceAClientConfig azureQueueConfig) {
        ...
        try {
            // creates directory/directories, if directory already exists, it will not throw
            // exception
    ...
    public Task uploadFile(Integer id, @NonNull MultipartFile file, Integer fileNumber) {
        ...
        QueueProcess queueProcess = new QueueProcess(azureQueueConfig);
        Task task = taskfind.get();
                    
        try {
            // upload file to azure blob storage
            if ((fileNumber >= FileServiceEnum.FIRSTSLOT.getSlotNumber())
                    && (fileNumber <= FileServiceEnum.THIRDSLOT.getSlotNumber())) {
                fileFilter(file);
                ...
                if (isImageFile(file)){
                    queueProcess.addQueueMessage(azureQueueConfig.getAccountName(), azureQueueConfig.getAccountKey(), "generation-pending-queue", generateQueueMessage(id, fileNumber, filenameURL));
                }                    
            } else {
                throw new InvalidParamException("The file number is not valid");
            }
        } catch (IOException e) {
    ...
    public String generateQueueMessage(Integer taskid, Integer slotNumber, String imagePath) {

        return String.format("{\"taskid\": \"%d\",\"slot\": \"%d\",\"image\":\"%s\" }", taskid, slotNumber, imagePath);
    }

    public boolean isImageFile(MultipartFile file) {
        return "image/jpeg".equalsIgnoreCase(file.getContentType());
    }

    private void fileFilter(MultipartFile file) {
       
        String enumValue = ValidMimeTypesEnum.getEnumByString(file.getContentType());
               
        if (file.getContentType() == null || file.isEmpty() || file.getSize() == 0 || enumValue == null || !EnumUtils.isValidEnum(ValidMimeTypesEnum.class, enumValue.toUpperCase())) {
    ...
    public void filePresent(String filename, Integer id) {
        ...
        if (taskfind.isPresent()) {
            Task task = taskfind.get();
            BlobContainerClient blobContainerClient = getBlobContainerClient(containerClient);
            BlockBlobClient blockBlobClient1 = blobContainerClient.getBlobClient(task.getAzureFile1())
                    .getBlockBlobClient();
            BlockBlobClient blockBlobClient2 = blobContainerClient.getBlobClient(task.getAzureFile2())
                    .getBlockBlobClient();
            BlockBlobClient blockBlobClient3 = blobContainerClient.getBlobClient(task.getAzureFile3())
                    .getBlockBlobClient();
            ...
            throw new InvalidParamException(taskNotPresentMessage);
        }
    }
    ...
    private HttpHeaders getSlotAndContent(HttpHeaders httpHeaders, Task task, Integer slot) throws URISyntaxException {
        ...
        if (slot == FileServiceEnum.FIRSTSLOT.getSlotNumber()) {
            ...
            if (task.getDownloadFile1() != null) {
                ...
                throw new NoSuchIdException(emptySlotMessage);
            }
